...
export const facilitySearchSchema = z.object({
  ...
  openBefore: z.string().regex(/^\d{2}:\d{2}$/).optional(),
  minRating: z.number().min(1).max(5).optional(),
  sort: z.enum(["distance", "price_asc", "price_desc", "rating_desc"]).default("distance"),
...
export const discoverSearchParamsSchema = z.object({
  ...
  sort: z.enum(["distance", "price_asc", "price_desc", "rating_desc"]).optional(),
