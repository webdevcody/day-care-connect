...
import { Button, cn } from "@daycare-hub/ui";
...
function ParentLayout() {
  ...
  useEffect(() => {
    if (dashboardLoading || !session) return;

    const onboardingCompleted =
      dashboardData?.onboardingCompleted ?? (session.user as any)?.onboardingCompleted ?? false;
    const isOnboardingPage = currentPath === "/parent/onboarding";
  ...
  }, [dashboardData, dashboardLoading, session, currentPath, navigate]);
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="flex flex-1 flex-col">
        ...
        <main className={cn("flex-1", !isOnboardingPage && "p-6")}>
          <Outlet />
        </main>
      </div>
    </div>
