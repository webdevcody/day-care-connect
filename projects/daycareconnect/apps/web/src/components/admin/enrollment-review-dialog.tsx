...
export function EnrollmentReviewDialog({
  ...
  // Reset internal state whenever the dialog opens or the enrollment changes
  useEffect(() => {
    if (open) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Review and approve or reject this enrollment application.</DialogDescription>
        </DialogHeader>

        {isDetailLoading || !enrollment ? (
          <div className="flex items-center justify-center py-12">
            <div className="text-muted-foreground">Loading enrollment details...</div>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">{enrollment.scheduleType.replace("_", " ")}</p>
              </div>
              <div>
            ...
            <div className="rounded-md border p-3">
              ...
              <p className="text-sm text-muted-foreground">{enrollment.parentEmail}</p>
              {enrollment.parentPhone && (
                <p className="text-sm text-muted-foreground">{enrollment.parentPhone}</p>
              )}
            </div>
